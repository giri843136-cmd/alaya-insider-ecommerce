...
export async function getBackupStats() {
  ...
  byType.rows.forEach((r: { type: string; count: number }) => { countsByType[r.type] = r.count; });

  const countsByStatus: Record<string, number> = {};
  byStatus.rows.forEach((r: { status: string; count: number }) => { countsByStatus[r.status] = r.count; });
