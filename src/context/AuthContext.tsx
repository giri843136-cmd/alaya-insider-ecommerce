...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const generateAndSendOTP = useCallback(() => {
    ...
    const backendUrl = getApiConfig().apiUrl;
    ...
    if (otpPhone) {
      fetch(`${backendUrl}/auth/admin/send-mobile-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone: otpPhone }),  // No code — backend generates securely
      })
        .then((res) => {
          if (!res.ok) log("auth.otp_sms_error", "system", `Backend SMS endpoint returned ${res.status}`);
          return res.json();
        })
        .then((data) => {
          if (data.success) {
  ...
  const verifyOTP = useCallback(async (code: string): Promise<boolean> => {
    ...
    const backendUrl = getApiConfig().apiUrl;
    try {
      const res = await fetch(`${backendUrl}/auth/admin/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          identifier,
