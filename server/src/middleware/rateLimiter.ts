...
export function createRateLimiter(options: Partial<RateLimitOptions> = {}) {
  ...
  return async (c: any, next: () => Promise<void>) => {
    // NEVER rate-limit OPTIONS preflight requests — always pass through
    if (c.req.method === "OPTIONS") {
      await next();
      return;
    }

    // NEVER rate-limit health check or system info endpoints
    const path = c.req.path;
    if (path.includes("/system/health") || path.includes("/system/info")) {
      await next();
      return;
    }

    // Skip if path doesn't match pattern
    if (opts.pathPattern && !opts.pathPattern.test(path)) {
      await next();
      return;
    }
