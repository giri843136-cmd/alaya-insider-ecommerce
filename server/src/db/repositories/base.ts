...
export abstract class BaseRepository<T extends { id: string }> {
  ...
  async bulkCreate(inputs: Partial<T>[], actor = "system"): Promise<T[]> {
    ...
    // ── Input validation ─────────────────────────────────────────
    // Detect null/undefined primary key values that would fail at SQL level
    const pkName = this.primaryKey;
    for (const input of inputs) {
      const pkVal = (input as Record<string, unknown>)[pkName];
      if (pkVal !== undefined && pkVal !== null && typeof pkVal !== "string") {
        throw new Error(
          `bulkCreate: primary key "${pkName}" must be a string or undefined, got ${typeof pkVal} (value: ${JSON.stringify(pkVal)})`,
        );
      }
    }

    // Process in batches using a single INSERT per batch
    for (let i = 0; i < inputs.length; i += BATCH_SIZE) {
      const batch = inputs.slice(i, i + BATCH_SIZE);
      const records = batch.map((input) => ({
        ...input,
        id: (input as any).id || this.genId(),
        created_at: now,
        updated_at: now,
      })) as Record<string, any>[];

      // Collect all unique column names across ALL records in the batch,
      // not just the first record. Records missing a column get NULL.
      ...
      const quotedColumns = columns.map((c) => this.quote(c)).join(", ");

      // Build multi-row VALUES clause
      const valueRows = records.map((_, rowIdx) => {
        const rowOffset = rowIdx * columns.length + 1;
        return `(${columns.map((_, colIdx) => `$${rowOffset + colIdx}`).join(", ")})`;
      });

      // Map each record's values, filling missing columns with null
      const values = records.flatMap((r) => columns.map((c) => (r as Record<string, any>)[c] ?? null));
      ...
      const result = await query<T>(sql, values);
      const entities = result.rows as unknown as T[];
    ...
    await createAuditLog({
      ...
      action: "create",
      entity_type: this.entityType,
      entity_id: `${results.length}_records`,
  ...
  async bulkUpdate(ids: string[], patch: Partial<T>, actor = "system"): Promise<number> {
    ...
    const setClause = setColumns.map((col, i) => `${this.quote(col)} = $${i + 2}`).join(", ");
    const setValues = Object.values(updates);

    // Update all IDs in a single query using = ANY($1)
    const sql = `
      UPDATE ${this.quote(this.tableName)}
      SET ${setClause}
      WHERE ${this.quote(this.primaryKey)} = ANY($1)
    `;

    const result = await query(sql, [ids, ...setValues]);
    ...
    await createAuditLog({
      ...
      action: "update",
      entity_type: this.entityType,
      entity_id: `${count}_records`,
      before_data: { count: ids.length },
      after_data: { count },
    });
  ...
  async bulkDelete(ids: string[], actor = "system"): Promise<number> {
    ...
    let sql: string;
    if (this.softDelete) {
      sql = `UPDATE ${this.quote(this.tableName)} SET deleted_at = NOW(), updated_at = NOW() WHERE ${this.quote(this.primaryKey)} = ANY($1)`;
    } else {
      sql = `DELETE FROM ${this.quote(this.tableName)} WHERE ${this.quote(this.primaryKey)} = ANY($1)`;
    }
   ...
   *
   * IMPORTANT: Use the provided `query` function inside the callback
   * to ensure all queries run within the transaction. Do NOT use
   * the pool-level `query()` from ../index.ts — those bypass transactions.
   */
  async transactional<R>(fn: (client: pg.PoolClient) => Promise<R>): Promise<R> {
