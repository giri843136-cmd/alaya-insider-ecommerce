...
export function executeBpmWorkflow(
  ...
  for (const action of sortedActions) {
    ...
    if (action.conditionExpression) {
      try {
        const fn = new Function("ctx", `return ${action.conditionExpression}`);
        if (!fn({ ...ctx, ...input })) {
        ...
        execution.steps.push({
          ...
          durationMs: 0,
          retries: 0,
          error: "Condition expression evaluation error",
        });
        continue;
      }
    }
...
export function transitionState(smId: string, event: string, ctx?: Record<string, string>): { success: boolean; fromState?: string; toState?: string; error?: string } {
  ...
  if (transition.conditionExpression && ctx) {
    try {
      const fn = new Function("ctx", `return ${transition.conditionExpression}`);
      if (!fn(ctx)) return { success: false, error: "Transition condition not met" };
    } catch {
      return { success: false, error: "Transition condition evaluation error" };
    }
  }
...
export function checkCompliance(workflowId: string): { compliant: boolean; violations: { rule: string; severity: string; message: string }[] } {
  ...
  for (const rule of rules) {
    try {
      const fn = new Function("wf", `return ${rule.checkExpression}`);
      if (!fn(wf)) {
